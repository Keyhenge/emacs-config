...
** Package manager
I use GuixSD, which is a distro that was inspired by NixOS, utilizes transactional package management, makes an effort to give reproducible builds, generally has a better UI/UX than NixOS, and only contains free software. The package manager behind it, Guix (as well as Nix!), can be installed on any Linux distro, and this gives an Emacs interface to it.
#+BEGIN_SRC emacs-lisp
  (use-package guix
    :ensure t
    :bind
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "H-c a") (lambda() (interactive)(find-file "~/agenda/agenda.org")))
#+END_SRC
