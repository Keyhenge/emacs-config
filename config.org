...
If you use OSX, the following binds fn to Hyper in Emacs. Consider using Karabiner Elements for further key changes, especially changling =caps_lock= to =control=.
...
*** mu4e
mu4e (mu-for-emacs) is a mailing suite inside of Emacs, allowing us to use Emac's text editing/movement when composing/reading email.
- =mu=, which actually contains mu4e
- =isync=, or mbsync which syncs a designated folder with your mailing server
mu4e is a bit special in that it requires both =mu= and =mbsync= to have already been installed, configured, and run. Check [[https://wiki.archlinux.org/index.php/Isync][here]] for a tutorial on setting up mbsync, and check [[http://www.djcbsoftware.nl/code/mu/mu4e/index.html#Top][here]] for the documentation on both mu and mu4e.
** Some notes
*** On use-package
...
This configuration is intended to be a full documented configuration for Emacs, and touches pretty much every aspect of it. However, if you don't like some portion of the config, nearly every section can be deleted individually without affecting the greater whole. Likewise, you're more than welcome to cherry pick sections into your own config.
* Interface
** Moving around Emacs
...
*** Rainbow
Any time you enter a hexidecimal that resembles a colorcode, it will automatically highlight the code with that color. See [[Dired Rainbow]] (while running this config in your Emacs) for an example.
 #+BEGIN_SRC emacs-lisp
