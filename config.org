...
#+BEGIN_SRC emacs-lisp
  ...
  (setq mu4e-maildir       (expand-file-name "~/maildir") ;; top-level Maildir
        ...
        mu4e-get-mail-command "mbsync gmail outlook" ;; Change these to whatever channels you use
...
** EMMS
EMMS allows for audio/video playback in Emacs by leveraging other programs, in this case MPD and MPV respectively. Use =H-p= to reach its various functions.
#+BEGIN_SRC emacs-lisp
  (use-package emms
    :ensure t
    :config
    ...
    :bind
    ("H-p p" . emms)
    ("H-p b" . emms-smart-browse)
    ("H-p r" . emms-player-mpd-update-all-reset-cache)
    ("H-<f5>" . emms-previous)
    ("H-<f8>" . emms-next)
    ("H-<f6>" . emms-pause)
    ("H-<f7>" . emms-stop))
